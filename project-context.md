...
DIDIT_ENV                 — ambiente de Didit: "mock" | "live". OBLIGATORIA para el adapter real. Sin ella: didit_env_unset, el adapter NO se construye y NO sale ninguna request (antes: caía en la PRODUCCIÓN de Didit por default → creaba verificaciones REALES con PII de personas reales y consumía cuota facturable). "live" exige NODE_ENV=production. ⚠️ NO existe "sandbox": Didit no publica un host de sandbox, su modo de prueba es el free tier sobre el MISMO host, separado por DIDIT_API_KEY + DIDIT_WORKFLOW_ID (setear "sandbox" tira didit_env_invalid, a propósito). ⛔ GATE DE RELEASE: setearla en el panel ANTES de deployar — al revés, un deploy con DIDIT_API_KEY seteada deja el adapter KYC tirando hasta que la env exista. Hoy el deploy es INERTE en este eje (Didit OFF, sin key → FallbackKycProvider), así que no hace falta setearla hasta activar Didit
DIDIT_BASE_URL            — override del host de Didit. Con DIDIT_ENV=mock es OBLIGATORIA (no hay host de mock canónico); con "live" es opcional y solo acepta hosts *.didit.me. NO define el ambiente y no puede contradecir a DIDIT_ENV (throw didit_base_url_env_conflict). Ya NO tiene default
